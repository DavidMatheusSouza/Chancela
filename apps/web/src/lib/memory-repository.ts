...
export async function seed(repo: Repository): Promise<void> {
  ...
  await createSeededAgent(repo, {
    ...
    // Seeded wallets are placeholders nobody holds a key for, so they claim no
    // provider. MERA and PRIVY are earned: by a proof of possession on /keys,
    // or by a Privy session -- and /integrations counts them as evidence.
