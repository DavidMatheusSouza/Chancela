 ...
 *
 * Source: 21st.dev, @hyperiux/typing-text. Two deviations from the published
 * component, both deliberate:
 *
 *  1. The root no longer forces `min-h-screen px-6 py-24`. Upstream is a
 *     full-page showpiece; here it sits inside a card on the landing page, and
 *     a Tailwind class appended through `className` cannot be relied on to beat
 *     a base class of equal specificity. Layout is the caller's job now.
 *  2. `startDelay` was added. It offsets every line, which lets two instances
 *     run back to back -- the prompt in muted text, then the verdict in the
 *     deny colour. Upstream paints all lines one colour, and a DENY that looks
 *     like everything else is the one thing this product may not ship.
 */
...
type TypingTextProps = {
  ...
  lineDelay?: number;
  /** Seconds added to every line's delay, so instances can be chained. */
  startDelay?: number;
  holdDuration?: number;
  blur?: number;
  rotate?: number;
...
/** Total seconds a sequential run takes -- used to chain a second instance. */
export function typingDurationOf(
  lines: string[],
  ...
  return lines.reduce((total, line) => {
    const chars = Math.max(Array.from(line).length, 1);
    const typing = Math.max(
      0.35,
      (chars / Math.max(charactersPerSecond, 1)) * Math.max(animationDuration, 0.1),
    );
    return total + typing + Math.max(lineDelay, 0);
  }, 0);
}
...
export default function TypingText({
  ...
  return (
    <section
      className={`typing-text-root relative flex w-full overflow-hidden ${backgroundColor ? "" : "bg-transparent"} ${textColor ? "" : "text-foreground"} ${className}`}
      style={{
      ...
      >
        {preparedLines.map((line, index) => {
          ...
          const delay =
            variant === "sequential"
              ? elapsed
              : startDelay + index * Math.max(lineDelay, 0);
          ...
          const lineStyle: TypingLineStyle = {
            ...
            "--typing-scale": scale,
            color: textColor,
            fontSize: `clamp(0.78rem, ${fontSize}vw, 0.92rem)`,
            fontWeight,
            lineHeight,
      ...
      <style>{`
        ...
        .typing-text-line[data-cursor-style] .typing-text-char::before {
          ...
          transition: background-color 240ms ease, border-color 240ms ease, border-radius 240ms ease,
            clip-path 240ms ease, height 240ms ease, opacity 180ms ease, transform 240ms ease, width 240ms ease;
        }
        ...
        .typing-text-line[data-typing-direction="right-to-left"]::after { inset-inline: auto calc(100% + 0.16em); }

        .typing-text-line[data-cursor-style="square"]::after,
        .typing-text-line[data-cursor-style="square"] .typing-text-char::before { width: 0.58em; height: 0.58em; }

        .typing-text-line[data-cursor-style="triangle"]::after,
        .typing-text-line[data-cursor-style="triangle"] .typing-text-char::before {
          width: 0.64em; height: 0.64em; clip-path: polygon(0 0, 100% 50%, 0 100%);
        }
        ...
        .typing-text-line[data-cursor-style="dot"] .typing-text-char::before {
          width: 0.38em; height: 0.38em; border-radius: 999px;
        }

        .typing-text-line[data-cursor-style="underscore"]::after,
        .typing-text-line[data-cursor-style="underscore"] .typing-text-char::before {
          inset-block-start: auto; inset-block-end: 0.16em; width: 0.72em;
          height: var(--typing-cursor-width); translate: 0 0;
        }

        @keyframes typing-text-write {
          0%   { width: 0; opacity: 0; filter: blur(var(--typing-blur)); transform: rotateX(var(--typing-rotate)) scale(var(--typing-scale)); }
          1%   { opacity: 1; }
          99.9%{ opacity: 1; }
          100% { width: min(var(--typing-target-width), 100%); opacity: 1; filter: blur(0); transform: rotateX(0deg) scale(1); }
        }

        @keyframes typing-text-loop {
          0%   { width: 0; opacity: 0; filter: blur(var(--typing-blur)); transform: rotateX(var(--typing-rotate)) scale(var(--typing-scale)); }
          6%   { opacity: 1; }
          46%, 72% { width: min(var(--typing-target-width), 100%); opacity: 1; filter: blur(0); transform: rotateX(0deg) scale(1); }
          100% { width: 0; opacity: 0; filter: blur(var(--typing-blur)); transform: rotateX(var(--typing-rotate)) scale(var(--typing-scale)); }
        }

        @keyframes typing-text-cursor { 0%,45% { opacity: 1; } 46%,100% { opacity: 0; } }
        @keyframes typing-text-cursor-finish { 0%,99.8% { opacity: 1; } 99.9%,100% { opacity: 0; } }
        @keyframes typing-text-fade-in { from { opacity: 0; } to { opacity: 1; } }

        @media (prefers-reduced-motion: reduce) {
          .typing-text-line {
            width: auto !important; max-width: 100%; opacity: 0;
            animation: typing-text-fade-in 0.4s ease-out forwards !important;
          }
          .typing-text-line::after { opacity: 0 !important; }
        }

        @keyframes typing-text-char-in { from { opacity: 0; } to { opacity: 1; } }
        @keyframes typing-text-caret { 0%,99.9% { opacity: 1; } 100% { opacity: 0; } }
        @keyframes typing-text-char-loop { 0%,62% { opacity: 1; } 62.01%,100% { opacity: 0; } }
        @keyframes typing-text-settle {
          0%   { opacity: 1; filter: blur(var(--typing-blur)); transform: rotateX(var(--typing-rotate)) scale(var(--typing-scale)); }
          100% { opacity: 1; filter: blur(0); transform: rotateX(0deg) scale(1); }
        }

        @media (max-width: 640px) {
          .typing-text-line {
            width: auto !important; max-width: 100%; white-space: normal !important;
            overflow-wrap: break-word; opacity: 1;
            animation: typing-text-settle var(--typing-duration) linear var(--typing-delay) both !important;
          }
          .typing-text-content { display: inline !important; overflow: visible !important; }
          .typing-text-char {
            position: relative; opacity: 0; animation: var(--typing-char-animation);
            animation-delay: calc(var(--typing-delay) + var(--char-index) * var(--typing-step));
          }
          .typing-text-line[data-cursor-style] .typing-text-char::before {
            content: ""; inset-inline-start: auto; inset-inline-end: calc(100% + 0.16em);
          ...
          .typing-text-line[data-cursor-style] .typing-text-char:first-child::before {
            animation-duration: var(--typing-delay); animation-delay: 0s;
          }
          .typing-text-line:not([data-typing-variant="loop"])[data-cursor-style]::after { content: none; }
          .typing-text-line[data-typing-variant="loop"][data-cursor-style]::after {
            position: static; display: inline-block; translate: none; margin-inline-start: 0.16em;
            vertical-align: -0.1em; animation-delay: calc(var(--typing-delay) + var(--typing-duration));
            animation-fill-mode: forwards;
          }
        }

        @media (max-width: 640px) and (prefers-reduced-motion: reduce) {
          .typing-text-line { opacity: 1 !important; animation: none !important; }
